...
The public installer is `https://toppk.github.io/omni/install.sh`. It detects the supported Linux or macOS architecture, downloads the matching latest release binary, fetches its release checksum, verifies SHA-256, and installs to `~/.local/bin` without `sudo`. It only replaces a target that identifies as an Omni release or local development build, rejects an unrelated `omni` already on `PATH`, skips an identical installed version, and warns if `~/.local/bin` is not on `PATH` without modifying shell configuration.
